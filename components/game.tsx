...
// Returns a multiplier: 0 = immune, 0.5 = not very effective, 2 = super effective
function typeEffectiveness(attackerTypes: string[], defenderTypes: string[]): number {
  ...
  for (const atkType of attackerTypes) {
    ...
    for (const defType of defenderTypes) {
      // Immunity = 0 damage, the best possible defense
...
// ── DEFENSIVE EFFECTIVENESS (how hard gym hits a mon) ─────────────────────────
// Checks if the gym type is IMMUNE to the mon's type (mon can't touch gym)
function defensiveEffectiveness(gymTypes: string[], monTypes: string[]): number {
  let multiplier = 1;
  for (const gymType of gymTypes) {
    const chart = TYPE_CHART[gymType];
    if (!chart) continue;
    let bestMult = 1;
    for (const monType of monTypes) {
      // If gym type is immune to mon type, mon can't hurt the gym at all
      if (chart.immune.includes(monType)) { bestMult = 0; break; }
      if (chart.strong.includes(monType)) bestMult = Math.max(bestMult, 2);
      else if (chart.weak.includes(monType)) bestMult = Math.min(bestMult, 0.5);
    }
    multiplier *= bestMult;
  }
  return multiplier;
...
function teamTypeAdvantage(
  ...
  for (const mon of team) {
    // Offensive: how well this mon hits the gym (super effective = big bonus)
    const offMult = typeEffectiveness(mon.types, gymTypes);
    // Defensive: how hard the gym hits this mon
    const defMult = typeEffectiveness(gymTypes, mon.types);
    // Immunity check: if mon is immune to gym type, huge defensive bonus
    const monIsImmune = defMult === 0;
    const gymIsImmune = offMult === 0;
    // Score in BST-equivalent points
    // Super effective hit = +40, immune to gym = +50, gym immune to us = -40
